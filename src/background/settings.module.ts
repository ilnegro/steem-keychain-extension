...
const sendBackImportedFileContent = async (fileContent: any) => {
  ...
  try {
    ...
    if (
      ...
      if (importedSettings.STEEM_ENGINE_ACTIVE_CONFIG) {
        await LocalStorageUtils.saveValueInLocalStorage(
          LocalStorageKeyEnum.STEEM_ENGINE_ACTIVE_CONFIG,
          importedSettings.STEEM_ENGINE_ACTIVE_CONFIG,
        );
      }

      if (importedSettings.STEEM_ENGINE_CUSTOM_ACCOUNT_HISTORY_API) {
      ...
      if (importedSettings.STEEM_ENGINE_CUSTOM_RPC_LIST) {
