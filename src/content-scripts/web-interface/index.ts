...
const setupInjection = () => {
  try {
    ...
    scriptTag.src = chrome.runtime.getURL('./steem_keychain.js');
    var container = document.head || document.documentElement;
    container.insertBefore(scriptTag, container.children[0]);
  } catch (e) {
    Logger.error('Steem Keychain injection failed.', e);
  }
};
setupInjection();
// Answering the handshakes
document.addEventListener('swHandshake', () => {
  window.postMessage(
    {
...
document.addEventListener('swRequest', (request: object) => {
  const prevReq = req;
