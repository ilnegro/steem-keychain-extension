...
const currency = Joi.string().valid('STEEM', 'SBD', 'TESTS', 'TBD').required();
...
const daily_pay = Joi.string()
  .regex(/^\d+(\.\d{3}) SBD$/)
  .required()
  .error(new Error('Wrong daily_pay format. Ex: "1.000 SBD"'));
...
const delegation = Joi.object({
  ...
  unit: Joi.string().required().valid('SP', 'VESTS', 'TP'),
  rpc,
});
