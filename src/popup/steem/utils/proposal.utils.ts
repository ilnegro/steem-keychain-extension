...
const hasVotedForProposal = async (
  ...
  const listProposalVotes = await SteemTxUtils.getData(
...
const getProposalList = async (
  ...
  const listProposals = await SteemTxUtils.getData(
    'condenser_api.list_proposals',
    [[-1], 1000, 'by_total_votes', 'descending', 'votable'],
  );

  const listProposalVotesRequestResult = await SteemTxUtils.getData(
    'condenser_api.list_proposal_votes',
  ...
  for (const p of listProposals) {
    ...
    proposals.push({
      ...
      receiver: p.receiver,
      dailyPay: dailyPay,
      link: `https://steempro.com/proposals/${p.proposal_id}`,
...
const isRequestingProposalVotes = async (globals: DynamicGlobalProperties) => {
  let dailyBudget = +(await ProposalUtils.getProposalDailyBudget());
  const proposals = (
    await SteemTxUtils.getData('condenser_api.list_proposals', [
      [-1, 0],
      100,
      'by_total_votes',
      'descending',
    ...
    proposal.totalVotes = FormatUtils.toSP(
...
const getProposalDailyBudget = async () => {
  return (
    parseFloat(
      (await AccountUtils.getExtendedAccount('steem.dao')).sbd_balance
        .toString()
        .split(' ')[0],
    ) / 100
  );
